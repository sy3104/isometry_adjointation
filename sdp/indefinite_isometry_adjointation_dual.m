% -------------------------------------------------------------------------
%
% File: indefinite_isometry_adjointation_dual.m
%
% Description :
...
function opt_fom = indefinite_isometry_adjointation_dual(din,dout,n,task,isComplex)
...
for index_alpha = 1:length(list_young_diagrams_in{N-1})
    ...
    for index_nu = 1:length(list_young_diagrams_out{N})
        nu = list_young_diagrams_out{N}{index_nu};
        for g = 1:factorial(N-1)
            PP = 0;
            for index_beta = 1:length(list_parent_out{N}{nu})
                beta = list_parent_out{N}{nu}{index_beta};
                PP = PP + Tensor(perm{N-1}{alpha}{g}, cycle{N}{nu}'*X{N}{beta}{nu}'*perm{N-1}{beta}{g}*X{N}{beta}{nu}*cycle{N}{nu});
            end
            PP*W{alpha}{nu}*PP' == W{alpha}{nu};
        end
    end
end

% ------------------------------------------------------------------
%          Dual quantum process conditions
% ------------------------------------------------------------------
...
for index_gamma = 1:length(list_young_diagrams_in{i-1})
    ...
    for index_beta = 1:length(list_young_diagrams_out{i})
        ...
        for index_alpha = 1:length(list_child_in{i-1}{gamma})
            ...
            for index_nu = 1:length(list_child_out{i}{beta})
                nu = list_child_out{i}{beta}{index_nu};
                XX = Tensor(X{i}{gamma}{alpha},X{i+1}{beta}{nu});
                WW{i-1}{gamma}{beta} = WW{i-1}{gamma}{beta} + XX*WW{i}{alpha}{nu}*XX'/din;
            end
        end
    end
end
for index_alpha = 1:length(list_young_diagrams_in{i})
    ...
    for index_beta = 1:length(list_young_diagrams_out{i})
        ...
        for index_nu = 1:length(list_child_out{i}{beta})
            nu = list_child_out{i}{beta}{index_nu};
            IX = Tensor(eye(dim{i}{alpha}),X{i+1}{beta}{nu});
            LHS = LHS + IX*WW{i}{alpha}{nu}*IX'/multin{i}{alpha};
        end
        for index_gamma = 1:length(list_parent_in{i}{alpha})
            gamma = list_parent_in{i}{alpha}{index_gamma};
            XI = Tensor(X{i}{gamma}{alpha},eye(dim{i}{beta}));
            RHS = RHS + XI'*WW{i-1}{gamma}{beta}*XI/multin{i-1}{gamma};
        end
        LHS == RHS;
    end
end
